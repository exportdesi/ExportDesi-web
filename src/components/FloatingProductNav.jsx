 ...
 * @param {string} showItemsOnId - If provided, the product row hides initially and shows when this ID is in view
 * @param {boolean} showGetQuote - Whether to show the Get Quote button
 * @param {boolean} simple - If true, shows only the label (primary text) for items
 */
export default function FloatingProductNav({ items, products, currentPath, categoryLabel = 'Related', hideOnId, showItemsOnId, showGetQuote = true, simple = true }) {
    const [isHovered, setIsHovered] = useState(false);
    const [isVisible, setIsVisible] = useState(true);
    const [isItemsVisible, setIsItemsVisible] = useState(!showItemsOnId);
    ...
    // Scroll-aware visibility for whole component
    useEffect(() => {
    ...
    // Scroll-aware visibility for items row specifically
    useEffect(() => {
        if (!showItemsOnId || typeof window === 'undefined') return;

        const observer = new IntersectionObserver(
            ([entry]) => {
                setIsItemsVisible(entry.isIntersecting);
            },
            { threshold: 0.1 }
        );

        const target = document.getElementById(showItemsOnId);
        if (target) observer.observe(target);
                ...
                {isVisible && (
                    ...
                    >
                        <div className="px-4 py-2">
                            {/* Related items row - conditionally visible */}
                            <AnimatePresence>
                                {relatedItems.length > 0 && isItemsVisible && (
                                    <motion.div
